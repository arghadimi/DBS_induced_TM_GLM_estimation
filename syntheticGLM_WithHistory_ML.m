...
param_stim.T=T;
param_stim.dt=dt;
param_stim.mode='Poiss';%% DBS or Poiss or Rand
...
xlim([0 10])
spike_mean=sum(V_bin(.2e5:1.2e5))
xlabel('Time(sec)')
...
% dt_sec = dt/1000; % sec
% % Time_Sim (msec)
% Length_Sim = floor(Time_Sim/dt);%length(fr);
% spikeMat = rand(Trial_num, Length_Sim) < dt_sec * repmat(fr,Trial_num,1);
% tVec = 0:dt:Time_Sim;
...
%% GlmFit 
Xdsgn =[XY,XX];
y=conv(V_bin,hann(40),'full'); 
y=y(1:length(V_bin));

AvgFR=sum(V_bin)/T;
wk=3*dt;
rate_ = KernelPSTH(V_bin,wk,dt,1);% smoothing
rate_=rate_/1330;
...
yes='y';
no='n';
response=no;%input('Do you want to plot the Mult-frequency time traces?(yes/no)');
